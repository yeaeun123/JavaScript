...
function testEverySome(){
    ...
    console.log("모든 인물의 나이가 25세 초과인가?", result); //false

    console.log("====some");
    // data 내부의 일부 객체의 나이가 25세 초과하는지 검증
    result = data.some(obj => obj.age > 25);
    console.log("일부 인물의 나이가 25세 초과인가?", result); // true
};
// testEverySome();
...
function testReduce(){
    ...
    }, 0 ); //(acc, value, index, arr)콜백함수/ 0 초기값
...
function testDataPipeline(){
    ...
    const filterdStudents= studentsWithTotal.filter(student => student.total >= 240);
    console.log("총점 240이상 filter: ", filterdStudents);

    // sort 함수 이용 -> 총점 기준으로 정렬
    // const sortedStudents = filterdStudents.sort((a, b) => a.total - b.total); // ASC
    const sortedStudents = filterdStudents.sort((a, b) => b.total - a.total); // DESC
    console.log("total순 정렬: ", sortedStudents);

    // reduce 함수 이용 -> 학생들의 총점 평균 구하기
    ...
    console.log("총점240이상 학생들의 총점 평균:", avgTotalScore);
